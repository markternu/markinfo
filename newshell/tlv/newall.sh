...
# ============================================================================
# read_and_remove_fixed_bytes 完整函数套件（TLV 版本）
# ============================================================================
# 包含的函数:
#   - read_and_remove_fixed_bytes  主函数（读取并移除TLV数据）
#   - 无其他依赖函数（完全独立）
# ============================================================================


# ============================================================================
# 功能：读取文件末尾TLV数据并移除（✅ TLV格式）
# 参数1: 文件路径
# 返回: 0=成功, 1=失败
# 输出: 读取到的文件名内容（通过 echo 输出到 stdout）
# ============================================================================
# 数据结构：
...
# ============================================================================
# 更新说明：
#   - 旧版本：读取固定1100字节 (100字节标志位 + 1000字节内容)
#   - 新版本：读取TLV格式数据 (Type 100B + Length 4B + Value NB)
#   - 支持任意长度的文件名，彻底解决中文字节数计算问题
# ============================================================================
function read_and_remove_fixed_bytes() {
    ...
    # 读取文件末尾104字节的数据块（包含完整的 Type + Length）
    ...
    # tail -c +105 表示从第105字节开始读取到末尾
    local content_string=$(printf "%s" "$full_tlv" | tail -c +105)
...
#
# # 示例1: 基本使用
# result=$(read_and_remove_fixed_bytes "/path/to/file.mp4")
# if [ $? -eq 0 ]; then
#     echo "读取到的文件名: $result"
# else
#     echo "读取失败"
...
#
# # 示例2: 在脚本中使用
# original_name=$(read_and_remove_fixed_bytes "$file_path" 2>/dev/null)
# if [ $? -eq 0 ] && [ -n "$original_name" ]; then
...
# # 示例3: 完整测试流程
# # 1. 创建测试文件
# echo "This is test content" > test.dat
#
# # 2. 写入TLV数据（需要先定义 write_fixed_bytes 函数）
# # write_fixed_bytes "【中文测试】文件名.mp4" test.dat
#
# # 3. 读取并移除TLV数据
# filename=$(read_and_remove_fixed_bytes test.dat)
# echo "提取的文件名: $filename"
#
# # 4. 验证文件已恢复
# cat test.dat  # 应该只显示: This is test content
# ============================================================================
...
# 函数依赖说明
# ============================================================================
# read_and_remove_fixed_bytes 函数是完全独立的，不依赖任何其他自定义函数
#
# 使用的系统命令:
#   - wc        : 计算文件大小
#   - tail      : 读取文件末尾数据
...
#   1. 文件路径为空
#   2. 文件不存在
#   3. 文件大小 < 104 字节（无法包含完整 TLV 头）
#   4. Length 字段为负数或超过文件剩余大小
#   5. 标志位不是 "FKY996"（文件未被标记）
#   6. 临时文件创建失败
#   7. 文件复制或替换失败
#
# 所有错误信息都输出到 stderr（>&2），不会污染 stdout 的返回值
# ============================================================================
...
# 性能说明
# ============================================================================
# 时间复杂度: O(n)，其中 n 是文件大小
#   - tail 命令: O(1)，只读取末尾固定字节
#   - head 命令: O(n)，需要读取整个文件（截断操作）
#
# 空间复杂度: O(n)
#   - 需要创建临时文件存储截断后的内容
#
# 优化建议:
#   - 对于超大文件（>1GB），可以考虑使用 truncate 命令替代 head+mv
