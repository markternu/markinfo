...
# 功能2：读取文件末尾1100字节并还原为字符串，然后删除这1100字节 - 修复中文字符问题
# 参数1: 文件路径
...
# 功能10：批量查看文件夹中文件的原始文件名 - 优化版（修复路径解析问题）
function batch_view_original_names() {
    ...
    if [ -z "$file_path_list_string" ]; then
        ...
        echo "示例: batch_view_original_names \"/Users/cc/Desktop/test/oppp/v{1..40}\""
        echo "      batch_view_original_names \"/v30\""
        echo "      batch_view_original_names \"/path/with spaces\" \"/another/path\""
        echo "      batch_view_original_names $'/path1\\n/path2\\n/path with spaces'"
        return 1
    fi
